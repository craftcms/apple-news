...
Craft.AppleNews.ArticlePane = Garnish.Base.extend({
  ...
  updatePane: function (newInfos) {
    for (var channelId in newInfos) {
      ...
      if (this.hasInfoChanged(channelId, "state", newInfos)) {
        ...
        if (
          $.inArray(newInfos[channelId]["state"], [
            "QUEUED_UPDATE",
            "PROCESSING",
            "PROCESSING_UPDATE",
            "LIVE",
          ...
          $menu.append(
            $(
              '<li><a data-action="copy-share-url" data-url="' +
                shareUrl +
                '">' +
                Craft.t("apple-news", "Copy share URL") +
                "</a></li>"
            )
          );
        }
        ...
        if (
          ...
          !this.draftId &&
          !this.revisionId &&
          newInfos[channelId]["canPublish"]
        ) {
          $menu.append(
            $(
              '<li><a data-action="publish-article">' +
                Craft.t("apple-news", "Publish to Apple News") +
                "</a></li>"
            )
          );
        }
        ...
        $menu.append(
          $(
            '<li><a href="' +
              downloadUrl +
              '" target="_blank">' +
              Craft.t("apple-news", "Download for News Preview") +
              "</a></li>"
          )
        );
  ...
  hasInfoChanged: function (channelId, property, newInfos) {
    return (
      typeof this.infos[channelId] == typeof undefined ||
      this.infos[channelId][property] != newInfos[channelId][property]
    );
  },
  ...
  refreshIfProcessing: function (delayed) {
    for (var channelId in this.infos) {
      ...
      if (
        $.inArray(this.infos[channelId]["state"], [
          "QUEUED",
          "QUEUED_UPDATE",
          "PROCESSING",
          "PROCESSING_UPDATE",
